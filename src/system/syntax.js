    ...
    for (var i=0; i<this.wrap.markSubsts.length; i++) {
      ...
      if (ms instanceof Mark) {
        ...
        if (subst.sym === sym && Mark.isSameMarks(subst.marks, marks)) {
          console.debug("getLabel", sym.name, subst.debugStr());
          return subst.label;
        }
      }
    }

    if (TRACE) console.error("getLabel notfound", BiwaScheme.to_write(this.expr), this.wrap.debugStr());
...
BiwaScheme.Syntax.INITIAL_ENV_ITEMS = [
  ...
  ["orr", "macro", function(so){
    //if (TRACE) console.log("orr", so.inspect());
    var sos = so.expose();
    var id = sos[0];
  ...
  ["swap!", "macro", function(so){
    //if (TRACE) console.log("swap!", so.inspect());
    // (swap! a b)
...
BiwaScheme.Expander = {
  ...
  _exp: function(so, env, menv) {
    if (TRACE && !BiwaScheme.isSelfEvaluating(so.expr)) console.log("_exp", so.inspect(), env, menv);
  ...
  _expandMacro: function(name, transformer, so) {
    if (TRACE) console.log("_expandMacro:" + name, so.inspect());
  ...
  _expandCore: function(coreTrans, so, env, menv) {
    if (TRACE) console.log("_expandCore", so.inspect());
  ...
  _expandExprs: function(exprs, env, menv) {
    //if (TRACE) console.log("_expandExprs", exprs);
...
var Syntax = BiwaScheme.Syntax,
    ...
    TRACE = BiwaScheme.Syntax.TRACE_EXPANSION,
